...
def get_genes_for_hgnc_id(ensembl, hgnc_symbol, build='grch37'):
    ...
    resp = ensembl.get(url, headers)
    return [x['id'] for x in json.loads(resp) if x['type'] == 'gene']

def get_transcript_ids_for_ensembl_gene_id(ensembl, gene_id, symbols, build='grch37'):
...
def get_previous_symbol(ensembl, hgnc_symbol):
...
def get_genomic_seq_for_transcript(transcript_id, expand, build='grch37'):
...
def get_cds_seq_for_transcript(transcript_id, build='grch37'):
    """ obtain the sequence for a transcript from ensembl
    """
...
def get_protein_seq_for_transcript(ensembl, transcript_id, build='grch37'):
    """ obtain the sequence for a transcript from ensembl
    """
    headers = {"content-type": "text/plain"}
...
def get_ranges_for_tx(transcript_id, feature, build='grch37'):
    ...
    ranges = []
    for exon in json.loads(resp.content):
...
def get_exon_ranges_for_transcript(transcript_id, build='grch37'):
    """ obtain the sequence for a transcript from ensembl
    """
    return get_ranges_for_tx(transcript_id, 'exon', build)

def get_cds_ranges_for_transcript(transcript_id, build='grch37'):
    """ obtain the sequence for a transcript from ensembl
    """
    return get_ranges_for_tx(transcript_id, 'cds', build)
